...
import { diets, type DietPlan } from "@/data/diets";
import { userDietPlanSchema, type UserDietPlan } from "./schema";

// Re-exported so the many existing importers don't all have to move.
export { userDietPlanSchema, type UserDietPlan };

const localUserDiets = createCollection(
  localStorageCollectionOptions({
...
export const userDietsFork = forkCollection({
  queryKey: "diets",
  local: localUserDiets,
  getKey: (plan) => plan.slug,
  fetch: async () =>
    (await fetchDocuments({ data: { kind: "diet" } })) as UserDietPlan[],
  upsert: (rows) => upsertDocuments({ data: { kind: "diet", rows } }),
  remove: (ids) => deleteDocuments({ data: { kind: "diet", ids } }),
});

/** Whichever collection backs the app right now — see `forkCollection`. */
export const userDiets = () => userDietsFork.active();
